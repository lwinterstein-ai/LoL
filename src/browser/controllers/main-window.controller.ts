...
export class MainWindowController {
  ...
  constructor(
    ...
    gepService.on('scoreboard-update', this.forwardScoreboardUpdate.bind(this));
    gepService.on('scoreboard-error', this.forwardScoreboardError.bind(this));
  ...
  private forwardScoreboardUpdate(scoreboard: any) {
    if (this.browserWindow?.isDestroyed() ?? true) {
      return;
    }
    this.browserWindow?.webContents?.send('league-scoreboard-update', scoreboard);
  }

  private forwardScoreboardError(payload: any) {
    if (this.browserWindow?.isDestroyed() ?? true) {
      return;
    }
    this.browserWindow?.webContents?.send('league-scoreboard-error', payload);
  }
