...
contextBridge.exposeInMainWorld('gep', {
  ...
  onScoreboardUpdate: (func) => {
    ipcRenderer.on('league-scoreboard-update', (_event, payload) => {
      func(payload);
    });
  },
  onScoreboardError: (func) => {
    ipcRenderer.on('league-scoreboard-error', (_event, payload) => {
      func(payload);
    });
  },
